...
public:
	...
	static string MinutesToHours(float Minutes)
	{
		if (Minutes < 60)
			return to_string((int)Minutes) + "min";

		float Remainder = Minutes / 60;
		short Hours = (short)Minutes / 60;
		short Mins = short((Remainder - Hours) * 60);
		...
		return to_string(Hours) + "h " + to_string(Mins) + "min";
	}
